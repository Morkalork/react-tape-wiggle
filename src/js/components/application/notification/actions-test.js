...
import {SHOW_INFO, SHOW_ERROR, showInfo, showError, hideInfo, hideError} from './actions';
...
test('hideInfo should return a type null notification', (assert) => {
  const result = hideInfo();

  assert.equals(result.payload.type, null, 'The type is set to null');
  assert.end();
});

test('hideError should return a type null notification', (assert) => {
  const result = hideError();
